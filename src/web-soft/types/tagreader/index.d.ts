...
interface TagReader {
    ...
    /**
     * Возвращает строку с текущим тегом в каноническом виде (скобки, атрибуты).
     * Если текущая позиция указывает не на именной тег, возвращает `undefined`.
     */
    ...
    /**
     * Возвращает строку, содержащую текущий тег в каноническом виде
     * (со всеми атрибутами, с правильно расставленными знаками кавычек и скобок).
     */
    GetTagStr(): string;
    ...
    /**
     * Считывает фрагмент HTML от текущего тега до закрывающего его сбалансированного тега.
     * Если тег одиночный, считывается сам тег.
     */
    ...
    /**
     * Запоминает в текущем `TagReader` прикрепленный файл (`CompoundAttc`)
     * для последующего его экспорта при помощи функции `ExportCompoundAttc`.
     ...
     * Следует учесть, что обычно `TagReader` используется только для чтения,
     * но если TagReader используется для обработки HTML-документов,
    ...
    /**
     * Осуществляет поиск определенного текста (неформатированного) не разделенного тэгами.
     * Если текст не найден, генерируется ошибка. После нахождения текста указатель остается
     * в конце текстового блока между тэгами, в котром находится найденый текст.
     * @param text - Текст, который нужно найти.
     */
    SkipToPlainText(text: string): TagReader;

    /**
     * Перемещает указатель на определенный тег с заданными атрибутами и оставляет указатель на начале этого тега.
     * В случае если атрибут необязателен (аргумент `isOptional` равен `true`),
     * то возвращает true при условии, что тег найден, и `false` - если не найден.
     * Если аргумент обязательный, и заданный тег не найден, вызывается исключение.
     ...
     * Перемещает указатель на определенный тег с заданными атрибутами и оставляет указатель на начале этого тега.
     * В случае если атрибут необязателен (аргумент `isOptional` равен `true`),
     * то возвращает `true` при условии, что тег найден, и `false` - если не найден. Если аргумент
